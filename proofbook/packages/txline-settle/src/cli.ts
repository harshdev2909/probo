...
import { DEVNET, MAINNET, type Network } from "./network";
import { TxLineSession, guestAuth } from "./session";
import { fixtures as fetchFixtures, scoresSnapshot } from "./feed";
import { fetchProofV3, findFinalisedSeq } from "./proof";
import {
  parlay,
  homeWin,
  overGoals,
...
import { reconstructReceipt } from "./receipt";
...
const c = {
  brass: (s: string) => (TTY ? `\x1b[33m${s}\x1b[0m` : s),
...
let JSON_MODE = false;
const out = (human: () => void, json: unknown) => {
  if (JSON_MODE) console.log(JSON.stringify(json, null, 2));
  else human();
};
const die = (msg: string, code = 1): never => {
  if (JSON_MODE) console.error(JSON.stringify({ error: msg }));
...
const cacheFile = (net: Network) =>
  path.join(cacheDir, new URL(net.apiOrigin).host + ".json");

function loadKeypair(flags: Args["flags"]): Keypair {
  const p =
...
function netOf(flags: Args["flags"]): Network {
  const base = flags.mainnet ? MAINNET : DEVNET;
  return flags.api ? { ...base, apiOrigin: String(flags.api) } : base;
}

function rpcOf(flags: Args["flags"], net: Network): Connection {
  const url =
    (flags.rpc as string) ??
    process.env.RPC_URL ??
    (net.cluster === "devnet"
...
function sessionOf(net: Network): TxLineSession {
  ...
  if (!apiToken)
    die(
      `no TxLINE session for ${net.apiOrigin} — run \`txline-settle auth\` first ` +
        `(a guest JWT alone cannot fetch proofs)`
    );
  return new TxLineSession({ origin: net.apiOrigin, jwt, apiToken });
}

// ── commands ─────────────────────────────────────────────────────────────────

async function cmdAuth(flags: Args["flags"]) {
  ...
  const subscribe = async (): Promise<string> => {
    ...
    const oracle = new anchor.Program(
      { ...txoracleIdl, address: net.oracleProgram },
      provider
    ) as any;
    ...
    if (!(await conn.getAccountInfo(ata))) {
      pre.push(
        createAssociatedTokenAccountInstruction(
          wallet.publicKey, ata, wallet.publicKey, mint, TOKEN_2022_PROGRAM_ID
        )
      );
    }
    const [pricing] = PublicKey.findProgramAddressSync(
      [Buffer.from("pricing_matrix")], oracle.programId
    );
    const [treasuryPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("token_treasury_v2")], oracle.programId
    );
    const vault = getAssociatedTokenAddressSync(
      mint, treasuryPda, true, TOKEN_2022_PROGRAM_ID
    );
  ...
  const session = new TxLineSession({ origin: net.apiOrigin, wallet, subscribe });
  await session.ensure();
  fs.mkdirSync(cacheDir, { recursive: true });
  fs.writeFileSync(
    cacheFile(net),
    JSON.stringify({ jwt: session.jwt, apiToken: session.apiToken }, null, 2)
  );
  out(
    () => {
...
async function cmdFixtures(flags: Args["flags"]) {
  const net = netOf(flags);
  const rows = await fetchFixtures(sessionOf(net), {
  ...
  out(
    () => {
      for (const f of rows.sort((a, b) => a.startTime - b.startTime)) {
        ...
        console.log(
          `  ${c.dim(when)}  ${String(f.fixtureId).padEnd(9)} ${f.participant1} v ${f.participant2}`
        );
      }
...
async function cmdScores(a: Args) {
  const fixtureId = Number(a.cmd[1]);
  if (!fixtureId) die("usage: txline-settle scores <fixtureId> [--watch]");
  ...
  if (a.flags.watch) {
    ...
    if (!res.ok || !res.body) die(`stream ${res.status}`);
      ...
      while ((i = buf.indexOf("\n\n")) >= 0) {
        ...
        try {
          ...
          for (const r of Array.isArray(rows) ? rows : [rows]) {
            if (r.FixtureId !== fixtureId) continue;
            if (JSON_MODE) console.log(JSON.stringify(r));
            else
              console.log(
                `  seq ${r.Seq}  status ${r.StatusId}  ${JSON.stringify(r.Score ?? {}).slice(0, 60)}`
              );
          }
        } catch {
          /* heartbeat */
  ...
  const rows = await scoresSnapshot(session, fixtureId);
  if (!rows.length)
    die(
      `TxLINE retains no score records for ${fixtureId} (scores age out after ~23 days). ` +
  ...
  out(
    () => {
      for (const r of rows.slice(-12)) {
        console.log(
          `  seq ${String(r.Seq).padEnd(6)} status ${String(r.StatusId).padEnd(4)} ${c.dim(new Date(r.Ts).toISOString())}`
        );
      }
      const fin = rows.filter((x: any) => x.StatusId === 100);
      const best = (fin.length ? fin : rows).reduce((m: any, x: any) =>
        (x.Seq ?? 0) > (m.Seq ?? 0) ? x : m
      );
      console.log(
...
async function cmdProof(a: Args) {
  const fixtureId = Number(a.cmd[1]);
  const keys = String(a.flags.stats ?? "1,2")
    .split(",")
    .map(Number);
  if (!fixtureId) die("usage: txline-settle proof <fixtureId> --seq <n> --stats 1,2");
  const net = netOf(a.flags);
  const session = sessionOf(net);
  const seq = a.flags.seq ? Number(a.flags.seq) : await findFinalisedSeq(session, fixtureId);
  const val = await fetchProofV3(session, fixtureId, seq, keys);
  out(
    () => {
      ...
      console.log(
        c.dim(
          `    multiproof: ${val.multiproof.hashes.length} shared hashes, indices [${val.multiproof.indices.join(",")}]`
        )
      );
...
function conditionOf(spec: string): Condition {
  const [name, arg] = spec.split(":");
  const line = arg ? Number(arg) : undefined;
  switch (name) {
    case "homeWin": return homeWin;
    case "overGoals": return overGoals(line ?? 2.5);
    case "overCorners": return overCorners(line ?? 9.5);
    case "overCards": return overCards(line ?? 3.5);
    default:
      return die(
        `unknown condition "${name}" — use homeWin | overGoals[:2.5] | overCorners[:9.5] | overCards[:3.5]`
      );
  }
}

async function cmdPredicate(a: Args) {
  if (a.flags.check) {
    // --check "1,2+1,2" → are these two leg sets combinable?
    const [x, y] = String(a.flags.check).split("+");
    const fam = (s: string) => s.split(",").map((k) => familyOf(Number(k)));
    const fa = new Set(fam(x));
    const clash = fam(y).filter((f) => fa.has(f));
    if (clash.length)
      die(
        `NOT combinable: both sides read the ${clash.join("/")} stat family. TxLINE evaluates ` +
  ...
  const aSpec = a.flags.a ? conditionOf(String(a.flags.a)) : homeWin;
  const bSpec = a.flags.b ? conditionOf(String(a.flags.b)) : overCorners(9.5);
  const market = parlay(aSpec, bSpec); // throws with the 6070 explanation on overlap
  out(
    () => {
      ...
      market.outcomes.forEach((o, i) =>
        console.log(`    ${i}: ${o.label}${i === 0 ? c.brass("   ← the parlay") : ""}`)
      );
    },
    market
  );
...
async function cmdVerify(a: Args) {
  let target = a.cmd[1];
  if (!target) die("usage: txline-settle verify <marketPda|settleTxSig> [--tamper]");
  const net = netOf(a.flags);
  const conn = rpcOf(a.flags, net);
  const session = sessionOf(net);
  const settlerId = new PublicKey(
    String(a.flags.program ?? (proofbookIdl as any).address)
  );
  ...
  if (target.length > 50) {
    ...
    if (!found) die("no market account found in that transaction");
    target = found!;
    if (!JSON_MODE) console.log(c.dim(`  resolved market ${target} from the transaction\n`));
  }
  ...
  if (JSON_MODE) {
    ...
    console.log(
      c.dim(
        "  TxLINE's on-chain program re-adjudicated this settlement against the root\n" +
    ...
    console.log(
      c.dim(
        "  One byte was corrupted and the oracle refused the proof. That is the point:\n" +
...
async function cmdMarket(a: Args) {
  const sub = a.cmd[1];
  const net = netOf(a.flags);
  const conn = rpcOf(a.flags, net);
  ...
  const prog = new anchor.Program(proofbookIdl as any, provider) as any;
  const pdaOf = (fixtureId: number, marketType: number, authority: PublicKey) =>
  ...
  const vaultOf = (market: PublicKey) =>
    PublicKey.findProgramAddressSync(
      [Buffer.from("vault"), market.toBuffer()],
      prog.programId
    )[0];
  const positionOf = (market: PublicKey, owner: PublicKey) =>
    PublicKey.findProgramAddressSync(
      [Buffer.from("position"), market.toBuffer(), owner.toBuffer()],
      prog.programId
    )[0];
  const marketArg = () => new PublicKey(String(a.flags.market ?? die("--market <pda> required")));

  switch (sub) {
    case "create": {
      const fixtureId = Number(a.flags.fixture ?? die("--fixture <id> required"));
      const marketType = Number(a.flags.type ?? 0);
      const mint = new PublicKey(String(a.flags.mint ?? die("--mint <usdcMint> required")));
      ...
      const sig = await prog.methods
        .initializeMarket(new BN(fixtureId), marketType, specs, 500, new BN(lockTime), new BN(21600), wallet.publicKey)
        .accounts({
    ...
    case "bet": {
      ...
      const amount = new BN(Math.round(Number(a.flags.amount ?? die("--amount <usdc> required")) * 1e6));
      // eslint-disable-next-line @typescript-eslint/no-var-requires
    ...
    case "settle": {
      ...
      const node = (n: any) => ({ hash: Array.from(Buffer.from(n.hash ?? n)), isRightSibling: !!n.isRightSibling });
      ...
      const { dailyRootsPda } = await import("./settle");
      const epochDay = Math.floor(tsMs / 86_400_000);
      const sig = await prog.methods
        .settleMarket(claimed, proof)
        .accounts({
          cranker: wallet.publicKey, market,
          oracleProgram: new PublicKey(net.oracleProgram),
          oracleRoots: dailyRootsPda(epochDay, new PublicKey(net.oracleProgram)),
        })
        .preInstructions([anchor.web3.ComputeBudgetProgram.setComputeUnitLimit({ units: 1_400_000 })])
        .rpc();
      out(
        () => console.log(c.green(`✓ settled trustlessly — outcome ${claimed} (${p1}-${p2})\n  tx ${sig}`)),
        { sig, claimed, proven: [p1, p2] }
      );
      break;
    }
    case "claim": {
    ...
    case "receipt": {
      const market = marketArg();
      const r = await reconstructReceipt({
...
async function main() {
  ...
  const cmd = a.cmd[0];
  try {
    if (!cmd || a.flags.help || cmd === "help") console.log(HELP);
    else if (cmd === "auth") await cmdAuth(a.flags);
    else if (cmd === "fixtures") await cmdFixtures(a.flags);
    else if (cmd === "scores") await cmdScores(a);
    else if (cmd === "proof") await cmdProof(a);
    else if (cmd === "predicate") await cmdPredicate(a);
    else if (cmd === "verify") await cmdVerify(a);
    else if (cmd === "market") await cmdMarket(a);
