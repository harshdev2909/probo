...
import { DEVNET, type Network } from "./network";
...
export interface VerifyProofOpts {
  anchor: any; // the caller's @coral-xyz/anchor module
  connection: Connection;
 ...
 */
export async function verifyProof(opts: VerifyProofOpts): Promise<{
  verified: boolean;
  provenValues: number[];
  epochDay: number;
  rootsPda: string;
  ...
  if (opts.tamper) {
    payload.leaves = payload.leaves.map((l: any, i: number) =>
      i === 0 ? { ...l, stat: { ...l.stat, value: l.stat.value + 1 } } : l
    );
  }
  ...
  if (!rootInfo) throw new Error(`TxLINE has not published a root for epoch day ${epochDay}`);
  if (!rootInfo.owner.equals(oracle))
    throw new Error(`roots account is owned by ${rootInfo.owner.toBase58()}, not the oracle`);

  const prog = readOnlyProgram(
    opts.anchor,
    opts.connection,
    { ...opts.txoracleIdl, address: net.oracleProgram },
    opts.payer ?? oracle
  );
  let verified = false;
  try {
    verified =
      (await prog.methods
        ...
        .preInstructions([
          opts.anchor.web3.ComputeBudgetProgram.setComputeUnitLimit({ units: 1_400_000 }),
        ])
        .view()) === true;
  } catch {
 ...
 */
export async function verifySettlement(opts: VerifySettlementOpts): Promise<VerifyOutcome> {
  ...
  try {
    if (isCombo) {
      ...
      const combo: any = await settler.account.comboSpec.fetch(comboPda);
