...
export async function findFinalisedSeq(session, fixtureId) {
 ...
 */
export async function fetchProofV3(session, fixtureId, seq, statKeys) {
    if (statKeys.length < 1 || statKeys.length > MAX_STAT_KEYS) {
        throw new Error(`${statKeys.length} stat keys requested; TxLINE accepts 1..${MAX_STAT_KEYS}.`);
    }
...
export function toPayloadV3(val, BN) {
    return {
...
export const provenValues = (val) => val.statsToProve.map((l) => l.stat.value);
/** The epoch day whose root this proof authenticates against. */
export const proofEpochDay = (val) => epochDayOf(val.summary.updateStats.minTimestamp);
