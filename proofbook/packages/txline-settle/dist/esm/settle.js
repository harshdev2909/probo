 ...
 */
export function dailyRootsPda(epochDay, oracleProgram = TXORACLE_DEVNET) {
    const b = Buffer.alloc(2);
    b.writeUInt16LE(epochDay & 0xffff, 0);
    return PublicKey.findProgramAddressSync([Buffer.from(DAILY_SCORES_SEED), b], typeof oracleProgram === "string" ? new PublicKey(oracleProgram) : oracleProgram)[0];
}
/**
 ...
 */
export async function rootIsPublished(connection, epochDay, oracleProgram = TXORACLE_DEVNET) {
    const pda = dailyRootsPda(epochDay, oracleProgram);
    const info = await connection.getAccountInfo(pda);
    if (!info)
        return false;
    const owner = typeof oracleProgram === "string" ? new PublicKey(oracleProgram) : oracleProgram;
