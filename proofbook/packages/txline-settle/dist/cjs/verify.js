...
exports.readOnlyProgram = readOnlyProgram;
 ...
 */
function readOnlyProgram(anchor, connection, idl, payer) {
...
async function verifyProof(opts) {
    const net = opts.network ?? network_1.DEVNET;
    const seq = opts.seq ?? (await (0, proof_1.findFinalisedSeq)(opts.session, opts.fixtureId));
    const val = await (0, proof_1.fetchProofV3)(opts.session, opts.fixtureId, seq, opts.statKeys);
    const payload = (0, proof_1.toPayloadV3)(val, opts.anchor.BN);
    if (opts.tamper) {
        payload.leaves = payload.leaves.map((l, i) => i === 0 ? { ...l, stat: { ...l.stat, value: l.stat.value + 1 } } : l);
    }
    const epochDay = (0, proof_1.proofEpochDay)(val);
    const oracle = new web3_js_1.PublicKey(net.oracleProgram);
    const roots = (0, settle_1.dailyRootsPda)(epochDay, oracle);
 ...
 */
async function verifySettlement(opts) {
    ...
    try {
        if (isCombo) {
            const [comboPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("combo"), market.toBuffer()], settler.programId);
