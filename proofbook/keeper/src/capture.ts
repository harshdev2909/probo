...
export async function capture(
  ...
  for (let hour = 0; hour < 24; hour++) {
    for (let interval = 0; interval < 12; interval++) {
      try {
        const batch = await client.scoresUpdates(epochDay, hour, interval, fixtureId);
        if (batch.length) {
          events.push(...batch);
          log.info(`+${batch.length} @ ${epochDay}/${hour}/${interval} (total ${events.length})`);
        }
      } catch {
        /* empty interval */
      }
  ...
  if (!events.length) throw new Error("no events captured — wrong epochDay or uncovered fixture");

  events.sort((a, b) => (a.Ts ?? a.ts ?? 0) - (b.Ts ?? b.ts ?? 0) || (a.Seq ?? a.seq ?? 0) - (b.Seq ?? b.seq ?? 0));
  const norm = events.map(normalizeUpdate).filter(Boolean) as any[];
  const finalised = norm.find((u) => u.statusId === 100);
  if (!finalised) throw new Error("no game_finalised (statusId=100) record found");
  const last = norm[norm.length - 1];
  log.info("finalised record", { seq: finalised.seq, ts: finalised.ts, score: finalised.score });

  const statKeys = cfg.statKeys.join(",");
  const proof = await client.statValidation(fixtureId, finalised.seq, cfg.statKeys as any);
  const p1 = proof.statsToProve[0], p2 = proof.statsToProve[1];
  log.info("real proof captured", { stats: JSON.stringify(proof.statsToProve) });
  ...
  const fixture = {
    ...
    provenance: {
      note:
        "Real TxLINE devnet data. This fixture was settled trustlessly on devnet via the live validate_stat_v2 CPI.",
      lastEvent: { seq: last.seq, score: last.score, statusId: last.statusId },
    },
  };

  const file = outFile || path.join(ROOT, "keeper", "fixtures", `${fixtureId}.json`);
