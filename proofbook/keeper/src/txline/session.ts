...
export class TxLineSession {
  ...
  constructor(private cfg: KeeperConfig, private store: Store, private chain: Chain) {
    this.api = axios.create({ baseURL: `${cfg.txlineApi}/api`, timeout: 30_000 });
    ...
    this.api.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (status === 403 && !rc._retriedToken) {
          ...
          this.log.warn("403 — apiToken rejected; re-running free subscribe + activate");
  ...
  async renewJwt(): Promise<string> {
    if (!this.renewing) {
      this.renewing = (async () => {
        const { data } = await axios.post(`${this.cfg.txlineApi}/auth/guest/start`);
  ...
  async subscribeAndActivate(): Promise<string> {
    ...
    const txoracleIdl = JSON.parse(
      fs.readFileSync(path.join(ROOT, "scripts", "vendor", "txoracle.json"), "utf8")
    );
    const txoracle = new anchor.Program(txoracleIdl, this.chain.provider) as any;
    ...
    if (!(await connection.getAccountInfo(ata))) {
      ...
      const tx = new Transaction().add(
        createAssociatedTokenAccountInstruction(
          wallet.publicKey, ata, wallet.publicKey, TXL_MINT,
    ...
    const [pricingMatrix] = PublicKey.findProgramAddressSync(
      [Buffer.from("pricing_matrix")], txoracle.programId);
    const [tokenTreasuryPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("token_treasury_v2")], txoracle.programId);
    ...
    this.store.saveSoon();
    this.log.info("apiToken activated", { apiToken: apiToken.slice(0, 14) + "..." });
    return apiToken;
  }
